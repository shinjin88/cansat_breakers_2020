...
class MPU9250:
    ...
    def configAK8693(self,mode='8HZ',bit='14bit'):
    ...
    def readMag(self):
        ...
        x = rawY + self.MagOffsetX
        y = rawX + self.MagOffsetY
        z = -(rawZ) + self.MagOffsetZ
...
class LibIMU:
    ...
    def getYawv2(self):
        ...
        while self.Thread_Flag:
            ...
            if (self.GYR_YAW > 140)or(self.GYR_YAW < -140):
                ...
                self.EA_YAW = self.GYR_YAW*0.8 + self.MGN_YAW*0.2
            prev_time = now_time
            time.sleep(self.SMPL_RT)
        return
    ...
    def launch6Axis(self,mode='angle'):
        ...
        if mode == 'angle':
            ...
            self.imuthread = threading.Thread(target=self.getPos_Yaw)
        elif mode == 'yaw':
